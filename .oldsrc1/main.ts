...
class Tools {
  ...
  public IsTransferJob<T extends Object>(obj: T): obj is TransferJob & T {
...
class MapInfo {
  ...
  public updateRoom(roomID: string) {
    ...
    if (controller?.my) {
      ...
      if (containers.length > 0) {
        roomMem.containers
          ? roomMem.containers[containers[0].id].rank == 1
            ? (roomMem.containers[containers[0].id].rank = 3)
            : undefined
          : undefined;
      }
    }
...
module.exports.loop = function () {
  ...
  for (let roomId in Game.rooms) {
    if (Game.time % 20 != 0) break;

    let room = Game.rooms[roomId];
    let terminalList = room.find(FIND_MY_STRUCTURES, {
    ...
    for (let orderID in Game.market.orders) {
      ...
      if (order.type == "sell" && order.resourceType == RESOURCE_HYDROGEN) selling = true;
    }
    if (!selling && terminal.store[RESOURCE_HYDROGEN] > 5000 && Game.market.credits > 500) {
    ...
    let orders = Game.market.getAllOrders(
      o => o.type == ORDER_BUY && o.resourceType == RESOURCE_HYDROGEN && o.price > 160 && o.price < 500
    );
    if (orders.length == 0 || terminal.store.getCapacity() / 4 > terminal.store.getUsedCapacity()) continue;
    ...
    let amount = Math.min(currentOrder.remainingAmount, terminal.store[RESOURCE_HYDROGEN]);
    while (
      Game.market.calcTransactionCost(amount, terminal.pos.roomName, currentOrder.roomName as string) >
    ...
    Game.market.deal(currentOrder.id, amount, terminal.pos.roomName);
  }
    ...
    for (let entity of states) {
      ...
      switch (entity.type) {
        case "creep":
          ...
          switch (entity.role) {
            ...
            case "hauler":
              ...
              break;
            case "pioneer":
              result = pioneer.run(entity, jobs);
              if (result == "deadCreep") {
                global.scheduler.stateUpdate++;
              }
              break;
        ...
        case "spawn":
          ...
          if (energy >= mhCost && mobileHarvesters.length < 9) {
          ...
          let haulers = _.filter(states, entity => entity.type == "creep" && entity.role == "hauler");
          creepCost = global.tools.BodyCost([CARRY, CARRY, MOVE]);
          if ((haulers.length < 8 && energy > creepCost * 3) || (haulers.length < 1 && energy > creepCost)) {
            let partList: BodyPartConstant[] = [];
            let partNum = energy / creepCost;
            let i = 1;
            while (i <= partNum && i < 8) {
          ...
          let pioneers = _.filter(states, entity => entity.type == "creep" && entity.role == "pioneer");
          let pioneerRequests = _.filter(jobs, job => job.type == "found");
          if (pioneers.length < 1 && pioneerRequests.length > 0) {
            let partList = [MOVE, CLAIM];
            let creepName = names[Math.round(Game.time / 20) % names.length] + (Game.time % 20) + "-pio";
            spawner.spawnCreep(partList, creepName);
            global.scheduler.stateUpdate++;
            global.scheduler.jobRest++;
          }
        ...
        case "tower":
          ...
          if (
            ...
            let damagedCreep = gameUnit.pos.findClosestByRange(FIND_MY_CREEPS, {
              filter: creep => creep.hits < creep.hitsMax
            });
            if (damagedCreep) gameUnit.heal(damagedCreep);
          }
          break;
        default:
