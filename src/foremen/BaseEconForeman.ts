...
abstract class BaseEconForeman extends BaseForeman {
    ...
    protected findBestTaskForJob(job: Job, simDwarf: Dwarf, simStore: {store:SimpleStore, used: number}, simPos:Pos, jobPossibleSources:{ roomName: string, id: Id<AnyStoreStructure> }[], dwarfPossibleSources:{ roomName: string, id: Id<AnyStoreStructure> }[]): TaskMatrixCell {
        ...
        for(const testSource of [...jobPossibleSources, ...dwarfPossibleSources]){
            ...
            if(test.score > best.score){
                best = test;
                plan = {visits: [{roomName:testSource.roomName, id: testSource.id, amount: actualAmount}], available: actualAmount, planUse: 0, resourceType};
            }
        }
    ...
    protected assignTasksByMatrix(jobAxis: Job[], dwarfAxis: Dwarf[]) {
        ...
        const tMatxDwarfwise: TaskMatrixDwarfwise[] = []
        {
            ...
            for(const dwarf of dwarfAxis){
                ...
                const dwarfRow = {dwarf, dwarfPossibleSources, simPos: creep.pos, simStore: {store: dwarf.info.cargo, used: 0}, time:0}
        ...
        for(const dWise of tMatxDwarfwise){
            ...
            for(const jWise of tMatxJobwise){
                const cell = this.findBestTaskForJob(jWise.job, dWise.dwarf, dWise.simStore, dWise.simPos, jWise.jobPossibleSources, dWise.dwarfPossibleSources);
        ...
        while((tMatxDwarfwise.length !== 0 && tMatxJobwise.length !== 0 && Game.cpu.bucket > 20)){
            ...
            tMatxDwarfwise[best.i].simStore.store[resource] = Math.max((tMatxDwarfwise[best.i].simStore.store[resource]||0)-claimedResourceAmount, 0)
            ...
            for(let i = 0; i < tMatxDwarfwise.length; i++){
                for(let j = 0; j < tMatxJobwise.length; j++){
                    ...
                    for(const source of taskMatrix[i][j].plan?.visits || []){
                        if((g.atlas.rooms[source.roomName]?.containers[source.id]?.store[(taskMatrix[i][j].plan?.resourceType||RESOURCE_ENERGY)] || -1) <= source.amount){
                            recompute = true
                            break;
                        }
                    }
                    ...
                    if (recompute){
                        taskMatrix[i][j] = this.findBestTaskForJob(tMatxJobwise[j].job, tMatxDwarfwise[i].dwarf, tMatxDwarfwise[i].simStore, tMatxDwarfwise[i].simPos, tMatxJobwise[j].jobPossibleSources, tMatxDwarfwise[i].dwarfPossibleSources);
                    }
                }
            }
        ...
        console.log(`cpu per creeps*jobs for ${this.name}: ${(Game.cpu.getUsed()-cpuStart)/(this.dwarves.length*this.jobs.length)}`)
