...
class EconCarryForeman extends BaseEconForeman {
  ...
  public getSpawnRequests(): SpawnRequest[] {
    ...
    const haulerBodies = [
      { body: [CARRY, MOVE], score: 1 },
      { body: [CARRY, CARRY, MOVE], score: 2 }
    ];

    const soonDeadCarryParts = this.dwarves
      .filter(d => d.role === "hauler" && (Game.getObjectById(d.id)?.ticksToLive ?? 0) < 500)
      .reduce((sum, d) => sum + d.info.carryParts, 0);
    const currentCarryParts = this.dwarves
      .filter(d => d.role === "hauler")
      .reduce((sum, d) => sum + d.info.carryParts, 0);
    ...
    for (const dwarf of this.dwarves.filter(
      d => d.role === "hauler" && (Game.getObjectById(d.id)?.ticksToLive ?? 0) < 200
    )) {
      requests.push({
        suffix: "h",
        urgency: 1,
