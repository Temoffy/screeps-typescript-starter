...
class EconWorkForeman extends BaseEconForeman {
  ...
  private updateTasks() {
    ...
    let dwarfOptions = this.dwarves.filter(
      dwarf => dwarf.role === "mobileDelver" && !dwarf.info.remove && !dwarf.commands[0]
    );
  ...
  public run() {
    for (const job of this.jobs) {
      const text: string[] = [];
      for (const key of ["type", "amount", "active", "priority"]) {
  ...
  public getSpawnRequests(): SpawnRequest[] {
    ...
    const energyDelverBodies = [
      { body: [W, M], score: 1 },
      { body: [W, W, M], score: 2 },
      { body: [W, W, W, W, W, M, M, M], score: 5 }
    ];
    ...
    let targetWorkParts = this.jobs
      .filter(j => j.type === "delve" && j.resourceType === RESOURCE_ENERGY)
      .reduce((sum, j) => sum + j.active, 0);
    targetWorkParts = Math.ceil(targetWorkParts * 1.5);

    const soonDeadWorkParts = this.dwarves
      .filter(d => d.role === "worker" && (Game.getObjectById(d.id)?.ticksToLive ?? 0) < 500)
      .reduce((sum, d) => sum + d.info.workParts, 0);
    const currentWorkParts = this.dwarves
    ...
    for (const dwarf of this.dwarves.filter(
      d => d.role === "worker" && (Game.getObjectById(d.id)?.ticksToLive ?? 0) < 200
    )) {
      requests.push({
        suffix: "w",
        urgency: 1,
